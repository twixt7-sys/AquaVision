// chart_payload -> build-time inline SVG. No libraries.
// Brand rules honoured here: gaps rendered as gaps (no interpolation),
// depth axes increase downward, series colours in brand order,
// thresholds dashed in warning/critical colours, status never colour-only.
import { esc, escAttr, humanize, provenanceBadge } from './html.mjs';

const SERIES_COLORS = ['var(--av-ocean)', 'var(--av-current)', 'var(--av-shallow)', 'var(--av-deep)', 'var(--av-slate)'];
const SEVERITY = {
  info: 'var(--av-slate)',
...
function annotationList(annos) {
  ...
  return `<ul class="anno-list">${annos
    .map((a) => {
      ...
      return `<li class="anno anno-${sev}"><span class="badge-icon">${sev === 'critical' ? '⚠' : sev === 'warning' ? '!' : 'ℹ'}</span>${esc(a.label || '')}</li>`;
    })
    .join('')}</ul>`;
}
...
function lineChart(doc) {
  ...
  let out = '';
  // bands first (annotation bands span the domain axis = x here)
  ...
  // threshold lines: dashed, severity colour, labelled (brand rule)
  for (const a of doc.annotations || []) {
    if (a.kind === 'hline') {
...
/* ------------- depth small multiples (shared depth axis, value per panel) ------------- */
...
function depthPanels(doc, seriesList) {
  ...
  const Yd = (v) => m.t + ((v - dmin) / (dmax - dmin)) * ih; // depth increases DOWNWARD
  ...
  seriesList.forEach((s, i) => {
    ...
    // depth bands span all panels' depth axis
    for (const a of doc.annotations || []) {
      if (a.kind === 'band' && Array.isArray(a.value)) {
    ...
    // point series: [value, depth]
    out += pathWithGaps(s.points.map((p) => ({ x: p.x, y: p.y })), Xv, Yd, color);

    // point labels attach to the panel containing the exact data point
    for (const a of doc.annotations || []) {
      if (a.kind === 'point_label' && Array.isArray(a.value)) {
        const [v, d] = a.value;
...
function categoricalLine(doc, seriesList) {
  ...
  const meta = seriesList[0].points.filter((p) => p.meta);
  const metaTable = meta.length
    ? `<details class="chart-data"><summary>Stage details</summary><div class="table-scroll"><table><thead><tr><th>Stage</th><th>Value</th><th>Detail</th></tr></thead><tbody>${seriesList[0].points
        .map((p) => `<tr><td>${esc(String(p.x))}</td><td class="num">${fmt(p.y)}</td><td>${esc(p.meta || '')}</td></tr>`)
        .join('')}</tbody></table></div></details>`
    : '';

  return svgWrap(W, H, out) + legend(seriesList.map((s) => s.name)) + annotationList((doc.annotations || []).filter((a) => typeof a.value === 'string')) + metaTable;
}
...
function barChart(doc) {
  ...
  return svgWrap(W, H, out) + (seriesList.length > 1 ? legend(seriesList.map((s) => s.name)) : legend([seriesList[0].name]));
}
...
function radarChart(doc) {
  ...
  return svgWrap(W, H, out) + legend(doc.series.map((s) => s.name)) +
    `<p class="muted">Scored 1–${maxV} (authors' qualitative judgement — see note above).</p>`;
}

/* ---------------- qualitative waterfall (spend envelopes) ---------------- */

const ENVELOPE = { low: 1, medium: 2, 'medium-high': 2.5, high: 3 };
...
function waterfallChart(doc) {
  ...
  data.forEach((d, i) => {
    ...
    const bw = Math.min(90, slot * 0.62);
    ...
    out += `<text class="tick" x="${x + bw / 2}" y="${m.t + ih + 16}" text-anchor="middle">${tspans(wrapText(label, 16).slice(0, 3), x + bw / 2, 0, 11)}</text>`;
  });
  out += `<line class="axis" x1="${m.l}" y1="${m.t + ih}" x2="${m.l + iw}" y2="${m.t + ih}"/>`;
  ...
  const unlocks = `<details class="chart-data" open><summary>What each phase unlocks</summary><div class="table-scroll"><table><thead><tr><th>Phase</th><th>Envelope</th><th>Unlocks</th></tr></thead><tbody>${data
    .map((d) => `<tr><td>${esc(String(d.phase || ''))}${d.kill_gate ? ' <span class="chip chip-critical"><span class="badge-icon">⚠</span>Kill gate</span>' : ''}</td><td>${esc(String(d.envelope || ''))}</td><td>${esc(String(d.unlocks || ''))}</td></tr>`)
    .join('')}</tbody></table></div></details>`;

  return svgWrap(W, H, out) + annotationList(doc.annotations) + unlocks;
}

/* ---------------- funnel (rendered as the honest empty-stage table) ---------------- */
...
function funnelChart(doc) {
  ...
  const rows = data
    .map((d, i) => {
      ...
      const value =
        d.value == null
          ? `<span class="chip chip-unknown"><span class="badge-icon">⌀</span>${esc(d.status || 'not computed')}</span>`
...
export function chartDataTable(doc, open = false) {
  const tables = (doc.series || [])
    ...
    .join('');
  if (!tables) return '';
  return `<details class="chart-data"${open ? ' open' : ''}><summary>Data table</summary>${tables}</details>`;
}
...
export function renderChartFigure(doc) {
  try {
    let body = null;
    ...
    switch (doc.chart_type) {
      ...
      case 'table': body = null; break; // caller's generic table renderer handles rows/columns
      default: body = null;
    }
    if (!body) return null;
    const provenance = doc.provenance ? `<div class="chart-prov">${provenanceBadge(doc.provenance)}</div>` : '';
    return `<figure class="chart">${provenance}${body}${includeTable ? chartDataTable(doc) : ''}</figure>`;
  } catch {
    return null; // fall back to generic rendering — never let a chart kill a page
  }
}

// Tiny sparkline for section-page cards.
export function sparkline(doc) {
  try {
    if (doc.chart_type !== 'line' || !doc.series?.length) return '';
    const { points, categorical } = parsePoints(doc.series[0].data);
    const ys = points.map((p) => (categorical ? p.y : p.y)).filter((v) => typeof v === 'number');
    ...
    const pts = points
      ...
      .filter(Boolean)
      .join(' ');
    return `<svg class="sparkline" viewBox="0 0 120 30" aria-hidden="true"><polyline points="${pts}" fill="none" stroke="var(--av-current)" stroke-width="1.6"/></svg>`;
  } catch {
    return '';
  }
}
